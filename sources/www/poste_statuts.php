...
*/
	...
	foreach ($list_app as $nom_poste=>$lp)
	{
		...
		if ($affichage==1)
		{
			...
			echo "<td align='center'><a href='app_parcs.php?Appli=".$lp["id"]."' style='color: ".$lnk."'>".$lp["name"]."</a></td>";
			...
			if (is_array($lp["parc"]))
			{
				foreach (@$lp["parc"] as $parc_app)
				{
					if ($i<>0)
						echo ", ";
					echo "<a href='parc_statuts.php?parc=".$parc_app."' style='color: ".$lnk."'>".$parc_app."</a>";
					$i++;
				}
			}
			if (is_array($lp["depend"]))
			{
				foreach ($lp["depend"] as $depend_app)
				{
					if ($i<>0)
						echo ", ";
					echo "<a href='app_parcs.php?Appli=".$depend_app."' style='color: ".$lnk."'>".$depend_app."</a>";
					$i++;
				}
			}
			if (@$lp["poste"])
