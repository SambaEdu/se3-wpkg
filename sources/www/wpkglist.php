...
$ErreurWget=false;
$url='http://deb.sambaedu.org/wpkg/se3/packages_stable.xml';
$fichier='se3_wpkglist.php';
$urlTest='http://deb.sambaedu.org/wpkg/se3/packages_testing.xml';
$fichiertest='se3_wpkglist.php?branch=testing';
exec ( "cd $wpkgwebdir;wget -N --timeout=30 --cache=off --tries=3 '$url' -O '$fichier' 2>&1", $output, $return_value);
if ( $return_value != 0 ) {
	$ErreurWget=true;
} else {
	exec ( "cd $wpkgwebdir;wget -N --timeout=30 --cache=off --tries=3 '$urlTest' -O '$fichiertest' 2>&1", $output, $return_value);
	if ( $return_value != 0 ) {
		$ErreurWget=true;
	} else {
