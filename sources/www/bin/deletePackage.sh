...
function installationsTimeStamp() {
	...
	xsltproc --output "$timeStampsXml" --stringparam op 'del' --stringparam Appli "$Appli" --stringparam TimeStamp "$TimeStamp" --stringparam user "$login" /var/www/sambaedu/wpkg/bin/timeStampAddPackages.xsl "$timeStampsXml"
}
