    ...
    if ($num_rows != 0) {
        while (mysqli_stmt_fetch($query)) {
            ...
            elseif ($res_type_entite == "parc")
                $tab[$res_id_app]["parc"][$res_nom_parc] = array(
                    "id_parc" => $res_id_parc,
                    "nom_parc" => $res_nom_parc,
                    "nom_parc_wpkg" => $res_nom_parc_wpkg
                );
        ...
        foreach ($list_app_dep as $id_app => $tmp) {
            ...
            if ($num_rows3 != 0) {
                while (mysqli_stmt_fetch($query3)) {
                    $tab[$res_id_app3]["info_app"] = array(
                        "id_app" => $res_id_app3,
...
function info_poste_rapport($nom_poste)
{
    ...
    if ($num_rows != 0) {
        while (mysqli_stmt_fetch($query)) {
            $tab[hash('md5', $res_id_nom_app)] = array(
                "id_nom_app" => $res_id_nom_app,
                "nom_app" => $res_nom_app,
...
function info_application_postes($id_nom_appli)
{
    ...
    if ($num_rows != 0) {
        while (mysqli_stmt_fetch($query)) {
            $tab[$res_nom_poste]["info_poste"] = array(
                "id_poste" => $res_id_poste,
                "nom_poste" => $res_nom_poste,
                "OS_poste" => $res_OS_poste,
                "date_rapport_poste" => $res_date_rapport_poste,
            ...
            if ($res_id_app == $id_app) {
                ...
                $tab[$res_nom_poste]["required_by"][$res_id_nom_app] = array(
                    "id_app" => $res_id_app,
                    "id_nom_app" => $res_id_nom_app,
                    "nom_app" => $res_nom_app
                );
    ...
    if ($num_rows2 != 0) {
        while (mysqli_stmt_fetch($query2)) {
            $tab[$res_nom_poste2]["info_poste"] = array(
                "id_poste" => $res_id_poste2,
            ...
            if ($res_id_app2 == $id_app) {
                $tab[$res_nom_poste2]["parc"][$res_nom_parc2] = array(
                    "id_parc" => $res_id_parc2,
                    "nom_parc" => $res_nom_parc2,
                    "nom_parc_wpkg" => $res_nom_parc_wpkg2
                );
            } else {
                $tab[$res_nom_poste2]["required_by"][$res_id_nom_app2] = array(
                    "id_app" => $res_id_app2,
