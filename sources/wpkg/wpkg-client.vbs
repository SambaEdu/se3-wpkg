...
'             - Lance l'exécution de wpkg-se4.js
...
'                  /debug      : Affiche en temps réel les msg lors de l'exécution de wpkg-se4.js.
...
Dim ServeurWpkg    : ServeurWpkg = "se4fs"  ' Serveur utilisé par les clients wpkg
Dim testCPU		   : testCPU = false	  ' test CPU désactivé par défaut à cause des soucis de détection sur les nouveaux CPU : 06/2013
'  Par défaut, le serveur se4 est utilisé pour déployer les applications.
'  Vous pouvez cependant préférer utiliser un autre serveur (par ex. si se4 est très chargé).
'  Pour utiliser le serveur 'NomDuServeur' à la place de 'se4', il faut :
'     - créer un partage nommé 'install' sur le serveur 'NomDuServeur'
'     - créer sur ce serveur un utilisateur 'adminse_name' ayant le mot de passe indiqué dans http://se4/setup (adminse_passwd)
'     - permettre à adminse_name d'écrire dans \\NomDuServeur\install\wpkg\rapports
'          (un accès en lecture suffit pour les autres dossiers et fichiers de ce partage)
'     - définir ci-dessus   ServeurWpkg = "NomDuServeur"   et enregistrer ce fichier (\\se4\install\wpkg\wpkg-client.vbs) ,
'     - recopier le contenu de \\se4\install dans \\NomDuServeur\install
'
'  De plus, pour que la gestion de la configuration des applis continue à se faire par l'interface web de se4,
'    Définir sur MonServeur un compte ADMINSE_NAME mot de passe PassWWWSE4 ayant un acces rw sur le partage \\NomDuServeur\install
'    puis sur le se4 :
...
'    Rmq. Le nom d'utilisateur ADMINSE_NAME et le mot de passe PassWWWSE4 doivent bien sûr être adapté selon votre imagination...
'
...
Dim watchDog : watchDog = 18000 ' Temps maxi en secondes pour l'exécution de wpkg-se4.js
...
If Not fso.FileExists(WPKGSE4JS) Then
   ...
   WScript.Quit 13
Else
   print "Script '" & WPKGSE4JS & "' présent."
End If
...
If InStr(GetTypeWin, "Server") > 0 Then
	...
	TooltipDelete() ' nettoyage des tooltips antérieurement déployés sur les OS serveurs
End If
...
WpkgSynchronize 'Exécute wpkg-se4.js pour synchroniser les applications
...
Function RemoveNetwork()
   ...
   End Function
Function WpkgSynchronize() 'Exécute wpkg-se4.js pour synchroniser les applications
   On Error Resume Next
   SetEnvironnement
...
Function TestCpuLoad
	' Calcul le taux d'occupation processeur du system consommé par les process sur une durée de 5 secondes
	' toutes les 30 secondes. Si ce taux d'occupation est en dessous de la charge maximum demandée pour lancer WPKG
	'la fonction sort de la boucle et retourne vrai. Au bout de 15 minutes de taux d'occupation supérieur
	'la fonction retourne faux.
	bExitFlag = False
	' Résultat par défaut.
	TestCpuLoad = False 
	' Délai entre deux check pour le calcul de performance (5 secondes).
	iDelayBetweenCheck = 5000
	' Délai entre deux tests de performance de charge (25 secondes).
	iWaitDelay = 25000
	...
	do
		...
		If iRetryCounter > iRetryCount Then
			Wscript.Echo "Durée maxi d'attente écoulée. Le PC est trop chargé."
			Exit Do
	    End if
...
Function TooltipDelete()
	' Suppression des tooltips : prévu pour nettoyer les OS Windows Server de Caen
	If fso.FileExists(MSGlog) Then
		fso.DeleteFile MSGlog, True
	End If
